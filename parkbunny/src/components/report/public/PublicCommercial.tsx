...
function getStreamEmoji(streamType: StreamType): string {
  switch (streamType) {
    ...
    case 'TESLA_DEMO': return '🚘'
    case 'WE_BUY_ANY_CAR': return '🏷️'
    case 'GIANT_WASHING_MACHINE': return '🧺'
    case 'DOG_GROOMING': return '🐕'
    case 'NHS_MRI_SCANNER': return '🏥'
    case 'FILM_CREW_HOSTING': return '🎬'
...
export function AlternativeRevenueStreams({ streams, formatCurrency }: AncillaryProps) {
  if (streams.length === 0) return null

  return (
    <section className="space-y-6">
      <Card className="border-amber-200/40 bg-gradient-to-br from-amber-50/50 to-transparent">
        <CardHeader>
          <CardTitle className="text-amber-800">Alternative Revenue Streams</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <p className="text-sm text-gray-700">Additional revenue opportunities available across the portfolio. All subject to site survey and feasibility assessment. These figures are <strong>not</strong> included in the portfolio total above.</p>
          ...
          <div className="rounded border p-4 text-xs text-gray-600">
            ...
            <p>All alternative revenue streams are subject to individual site surveys and feasibility assessments. Revenue figures are indicative estimates only and may vary based on location, footfall, and market conditions.</p>
          </div>
        </CardContent>
      </Card>
    </section>
